...
use frame_support::{assert_noop, assert_ok, parameter_types, traits::Randomness};
use sp_core::H256;
use sp_runtime::{
    testing::Header,
...
fn can_create() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            last_event(),
            Event::kitties(crate::Event::<Test>::KittyCreated(100, 0, kitty))
        );
    });
}
...
fn can_breed() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            last_event(),
            Event::kitties(crate::Event::<Test>::KittyBred(100u64, 2u32, kitty))
        );
    });
}
